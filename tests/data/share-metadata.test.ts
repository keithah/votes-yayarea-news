...
test("share metadata helper normalizes canonical URLs and static share image URLs", () => {
  assert.equal(normalizeCanonicalPath("/races/california-governor"), "/races/california-governor/");
  assert.equal(normalizeCanonicalPath("races/california-governor?draft=1"), "/races/california-governor/");
  assert.equal(normalizeCanonicalPath("/"), "/");
  assert.equal(absoluteUrl("/races/california-governor"), `${SITE_ORIGIN}/races/california-governor/`);
...
test("entity metadata describes the public source trail and evidence counts", async () => {
  const metadata = await generateEntityMetadata({ params: Promise.resolve({ slug: "california-governor-akinyemi-agbede" }) });

  assert.equal(metadata.title, "Akinyemi Agbede public source trail");
  assert.equal(metadata.description, "Akinyemi Agbede public source trail across 1 races: 1 tracked source records and 1 evidence items.");
  assertMetadataCommonFields(metadata, "/entities/california-governor-akinyemi-agbede/");
  assertSafeMetadata(metadata);
});
...
test("missing descriptions and custom unsafe inputs cannot leak local paths into social tags", () => {
  const metadata = buildShareMetadata({
    ...
    title: "manual/reviews/races/california-governor.json",

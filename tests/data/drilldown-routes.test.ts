...
test("entity and source routes statically generate public recommendation slugs only", async () => {
  ...
  assert.equal(entityParams.length, 63);
  ...
  assert.deepEqual(sourceParams.map((param) => param.slug).sort(), ["california-secretary-of-state", "growsf-voter-guide", "san-francisco-chronicle-editorial-board"]);
...
test("source page model exposes public recommendation receipts, diagnostics, and related links", async () => {
  ...
  assert.equal(model.counts.relatedRaceCount, 2);
  assert.equal(model.counts.publicPositionCount, 62);
  assert.equal(model.counts.evidenceCount, 62);
  assert.equal(model.counts.entityCount, 62);
  assert.deepEqual(model.relatedRaces.map((race) => `/races/${race.slug}/`).sort(), ["/races/california-governor/", "/races/state-assembly-district-17/"]);
  ...
  assert.equal(model.diagnostics.relatedRaceCount, 2);

...
## Regenerate and validate ingestion
...
`pnpm verify:s04` also checks that the representative S03 artifact/chunk files exist before extraction. If a path is missing, rerun ingestion with the command above.

## Layout

- `raw/*.html` — original fixture or fetched source body for audit and manual fallback.
...
S04 commands consume `artifacts/*.json` and `chunks/*.json` through `data/ingestion/manifest.json`:
...
The deterministic fixture provider is network-free and writes hidden drafts plus diagnostics under `data/extracted/`. Optional live extraction is explicit:
...
Never persist or print API keys, authorization headers, or raw provider responses that may contain secrets. Hidden generated drafts in `data/extracted/` and editable review files in `manual/reviews/` are not public data. They become public only when `pnpm review:positions publish --race-slug mayor` copies verified/public review records into `manual/overrides/races/` and public validation passes.

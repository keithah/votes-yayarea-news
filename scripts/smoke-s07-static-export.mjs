...
export const DEFAULT_ASSET_SAMPLE_LIMIT = 40;
...
export class SmokeFailure extends Error {
  constructor(phase, message, diagnostics = {}) {
    super(`[smoke-s07-static-export] Phase ${phase}: ${message}`);
...
export function choosePort(env = process.env) {
  const raw = env.S07_SMOKE_PORT;
  if (!raw) return 0;
  const port = Number(raw);
  if (!Number.isInteger(port) || port < 1024 || port > 65535) {
    throw phaseFail("server-start", `invalid S07_SMOKE_PORT ${JSON.stringify(raw)}; expected integer 1024-65535.`, { envVar: "S07_SMOKE_PORT", value: raw });
  }
  return port;
}
...
export function selectRoutesForSmoke(routes, { exhaustive = true, sampleLimit = DEFAULT_ROUTE_SAMPLE_LIMIT } = {}) {
  const normalized = dedupeRoutes(routes);
  if (exhaustive) return normalized;
  ...
  for (const className of ["homepage", "race", "source", "entity", "disclosure"]) add(normalized.find((route) => route.className === className));
...
function dedupeRoutes(routes) {
  ...
  for (const contract of routes ?? []) {
    ...
    seen.add(route);
    normalized.push({ ...contract, route });
  }
  return normalized;
}
...
export function selectAssetSample(routeHtmlChecks, limit = DEFAULT_ASSET_SAMPLE_LIMIT) {
  ...
  for (const routeCheck of routeHtmlChecks) {
    for (const asset of routeCheck.assets ?? []) {
      ...
      seen.add(asset);
      assets.push({ asset, sourceRoute: routeCheck.route });
...
export function validateS07StaticSmokeReport(report) {
  const errors = [];
  ...
  if (report.generatedBy !== SMOKE_GENERATOR) errors.push("generatedBy must identify the S07 smoke script");
  ...
  if (!report.counts?.routeClasses || typeof report.counts.routeClasses !== "object") errors.push("counts.routeClasses must be present");
  ...
  for (const asset of report.assetChecks ?? []) {
    ...
    if (asset.expectedType && !String(asset.contentType ?? "").toLowerCase().startsWith(asset.expectedType)) errors.push(`asset ${asset.asset ?? "<missing>"} must return ${asset.expectedType}`);
  }
...
export async function runS07StaticSmoke({ projectRoot = process.cwd(), outDir = resolve(projectRoot, DEFAULT_OUT_DIR), jsonOut = null, port = choosePort() } = {}) {
  if (!existsSync(outDir) || !statSync(outDir).isDirectory()) {
    throw phaseFail("preflight", "missing out/ directory. Run pnpm build before S07 static smoke.", { outDir: path.relative(projectRoot, outDir).replaceAll(path.sep, "/") || DEFAULT_OUT_DIR });
  }

  const contracts = loadS07RouteContracts(projectRoot);
  if (contracts.issues.length > 0) throw phaseFail("route-discovery", contracts.issues.join("; "));
  const routes = selectRoutesForSmoke(contracts.routes, { exhaustive: true });
  ...
  const report = {
    ...
    redirectChecks: [],
    assetChecks: [],
    counts: { checkedRoutes: 0, redirectChecks: 0, assetChecks: 0, routeClasses: {} },
    phases: {
      preflight: { status: "pass" },
      routeDiscovery: { status: "pass", classCounts: contracts.classCounts, coverageRaceContracts: contracts.coverageContracts.length, discoveredRoutes: contracts.routes.length, selectedRoutes: routes.length, mode: "exhaustive" },
      serverStart: { status: "pass" },
      routeFetch: { status: "pending" },
      redirectChecks: { status: "pending" },
      assetChecks: { status: "pending" },
      smokeReport: { status: "pending" },
  ...
  try {
    for (const contract of routes) {
      ...
      const missingText = response.status === 200 ? (contract.requiredText ?? []).filter((value) => !text.includes(value)) : [];
      const missingPatterns = response.status === 200 ? (contract.requiredPatterns ?? []).filter((pattern) => !pattern.test(text)).map((pattern) => String(pattern)) : [];
      const check = { route: contract.route, className: contract.className, status: response.status, contentType, bytes, assets, missingText, missingPatterns };
      ...
      if (missingText.length > 0) throw phaseFail("route-marker", `${contract.route} is missing required text ${missingText.map((value) => JSON.stringify(value)).join(", ")}.`, report);
      if (missingPatterns.length > 0) throw phaseFail("route-marker", `${contract.route} is missing required markers ${missingPatterns.join(", ")}.`, report);
    }
    ...
    for (const contract of routes.filter((route) => route.route !== "/")) {
      ...
      const check = { route: noSlash, className: contract.className, status: response.status, location, expectedLocation: contract.route };
    ...
    for (const assetCheck of selectAssetSample(report.checkedRoutes)) {
      const { asset, sourceRoute } = assetCheck;
      ...
      const check = { sourceRoute, asset, status: response.status, contentType, expectedType, bytes };
      report.assetChecks.push(check);
      if (response.status === 404 || response.status < 200 || response.status >= 400) throw phaseFail("asset-fetch", `${asset} from ${sourceRoute} returned ${response.status}.`, report);
      ...
      if (bytes <= 0) throw phaseFail("asset-fetch", `${asset} from ${sourceRoute} returned an empty asset response.`, report);
    }
    ...
    report.counts = {
      ...
      routeClasses: report.checkedRoutes.reduce((counts, route) => ({ ...counts, [route.className]: (counts[route.className] ?? 0) + 1 }), {}),
    };
    report.phases.smokeReport = { status: "pending" };

    const validationErrors = validateS07StaticSmokeReport(report);
    if (validationErrors.length > 0) throw phaseFail("smoke-report", validationErrors.join("; "), report);
    report.phases.smokeReport = { status: "pass" };
...
async function main() {
  try {
    ...
    const report = await runS07StaticSmoke({ jsonOut });
